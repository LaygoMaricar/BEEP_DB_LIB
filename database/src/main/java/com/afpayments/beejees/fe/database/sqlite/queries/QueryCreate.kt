...
class QueryCreate {
    companion object{
        ...
        const val CARD_TRANSACTIONS_TABLE = "CREATE TABLE `${CardTransactionsTable.TABLE_NAME}` ( `${CardTransactionsTable.MTC}` INTEGER AUTOINCREMENT, `${CardTransactionsTable.TRANSACTION_DATA}` TEXT, `${CardTransactionsTable.DATE_INSERTED}` TEXT, PRIMARY KEY(`${CardTransactionsTable.MTC}`) )"
        const val TRANSACTIONS_TABLE = "CREATE TABLE `${TransactionsTable.TABLE_NAME}` ( `${TransactionsTable.MTC}` INTEGER AUTOINCREMENT, `${TransactionsTable.TRANSACTION_TYPE}` TEXT, `${TransactionsTable.TRANSACTION_DATA}` TEXT, `${TransactionsTable.DATE_INSERTED}` TEXT, PRIMARY KEY(`${TransactionsTable.MTC}`) )"
        ...
        const val PRINT_TEMPLATE_TABLE = "CREATE TABLE `${PrintTemplatesTable.TABLE_NAME}` ( `${PrintTemplatesTable.ID}` INTEGER UNIQUE, `${PrintTemplatesTable.DATA}` TEXT, `${PrintTemplatesTable.TYPE}` TEXT, PRIMARY KEY(`${PrintTemplatesTable.ID}`) )"
        const val CASHBOX_REPORTS_TABLE = "CREATE TABLE IF NOT EXISTS  `${CashboxReportsTable.TABLE_NAME}` ( `${CashboxReportsTable.MTC}` INTEGER PRIMARY KEY AUTOINCREMENT, `${CashboxReportsTable.REPORT_DATA}` TEXT, `${CashboxReportsTable.DATE_INSERTED}` TEXT )"
    }
}